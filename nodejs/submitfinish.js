...
var stepReleaseP1 = 0;
var stepReleaseP2 = 0;
var finalStepReleaseP1 = 0;
var finalStepReleaseP2 = 0;
var keyProvider = "";    
var eos = "";    
...
function getTableData() {    
    ...
    eos.getTableRows({
        ...
        // calculate the amount to release to person1 for this step
        if (totalSteps > 0) {
            finalStepReleaseP1 = currentBalanceP1;
            ...
            finalStepReleaseP1 = 0;
        }

        // calculate the amount to release to person2 for this step
        if ((totalSteps > 1) && (currentSteps > 0)) {
            finalStepReleaseP2 = currentBalanceP2;
            // validate the decimals - 4 units of precision
        ...
        // validate the step number is the last step
        ...
        if (outputMessage == "success") {                                
            checkWallet(currentBalanceP1,currentBalanceP2,finalStepReleaseP1,finalStepReleaseP2,person1,person2);
        } else {            
            finalReturn(outputMessage);
        }
...
function checkWallet(currentBalanceP1,currentBalanceP2,finalStepReleaseP1,finalStepReleaseP2,person1,person2) {
    ...
    function validateBalance(res) {    
        ...
        if (res) {
            ...
            if ( Math.abs(Number(finalBalance.toFixed(4)) - Number(expectedBalance.toFixed(4))) < 0.00001 ) {                                    
                updateContract(finalStepReleaseP1,finalStepReleaseP2,person1,person2);
            } else {
                // balance error
                finalReturn("error32");
            }
...
// update contract
function updateContract(finalStepReleaseP1,finalStepReleaseP2,person1,person2) {
    ...
    .then((contract) => {        
        ...
        .then(trx => {
            ...
            transferFunding1(finalStepReleaseP1,finalStepReleaseP2,person1,person2);
        }).catch(e => {            
...
function transferFunding1(finalStepReleaseP1,finalStepReleaseP2,person1,person2) {    
    // convert to string rounded to 4 decimals
    var release = finalStepReleaseP1;    
    if (parseFloat(release) <= 0) {
        // skip this person, nothing to release
        transferFunding2(finalStepReleaseP1,finalStepReleaseP2,person1,person2);
    } else {
...
function transferFunding2(finalStepReleaseP1,finalStepReleaseP2,person1,person2) {    
    // convert to string rounded to 4 decimals - subtract any fee to matcheos
    var release = finalStepReleaseP2;
    if (parseFloat(release) <= 0) {
        // skip this person, nothing to release
        finalReturn(outputMessage);
    } else {

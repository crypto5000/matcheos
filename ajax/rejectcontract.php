...
$rejectFrom = validate($rejectFrom);
...
// validate the rejectFrom is yes or no
if (($rejectFrom == "no") || ($rejectFrom == "yes")) {
    // do nothing
} else {
...
if ($statusCode > 0) {
    ...
    if ($stmt = mysqli_prepare($mysqli, $sql)) {
        ...
        if ($numRows < 1) {
			...
			$logging = "There was a hack attempt on rejectcontract at: ";
            $logging .= $createDate;
    ...
    if ($statusCode == 0) {
        ...
        if ($stmt = mysqli_prepare($mysqli, $sql)) {
            ...
            if ($numRows < 1) {
                ...
                $to = ADMINEMAIL;  
                $subject = 'REJECT CONTRACT FAIL: NO DATA FOUND ON PAGE';
    ...
    if ($statusCode == 0) {
        ...
        $sql = "SELECT wastID, spotID1, spotID2, offer1, offer2, status1, status2, contractType, contractSteps FROM contractTable WHERE (spotID1 = ? OR spotID2 = ?) AND contractID = ? AND aliveFlag = ? AND deleteFlag = ? AND errorFlag = ?";
        ...
        if ($stmt = mysqli_prepare($mysqli, $sql)) {
            ...
            if ($numRows < 1) {
                ...
                // bind result variables
                mysqli_stmt_bind_result($stmt, $wastIDBind, $spotID1Bind, $spotID2Bind, $offer1Bind, $offer2Bind, $status1Bind, $status2Bind, $contractTypeBind, $contractStepsBind);
    ...
    if ($statusCode == 0) {
        ...
        if ($stmt = mysqli_prepare($mysqli, $sql)) {
            ...
            if ($numRows < 1) {
                ...
                // verify only 1 step or less occurred (currentStep leads by 1 for the next step)
                if ($currentStep > 2) {
                    // not valid
                    $statusCode = 24;
                }
    ...
    if ($statusCode == 0) {
        ...
        if (trim($rejectContract) == "success") {
            ...
            // send an email to the admin         
            $to = ADMINEMAIL;  
            $subject = 'REJECT CONTRACT FAIL: ERROR IN NODEJS QUERY TRANSFER';
    ...
    // if still ok - only for rejectFrom "no"
    if (($statusCode == 0) && ($rejectFrom == "no")) {
    ...
    if ($statusCode == 0) {
        ...
        // update the reject flag for match - only for rejectFrom "no"
        if ($rejectFrom == "no") {
